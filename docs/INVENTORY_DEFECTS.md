...
| P0-2 | `api/routes/lipsync.py` namespace `lipSync.*` vs renderer `lipsync.*` | **partially closed** by PR #101 (`lipSync.getProviders` caller rename). Still open: 4 lowercase calls have NO backend handler at all — `lipsync.start`/`.cancel`/`.listJobs`/`.getJob` need **new** backend handlers, not renames. Core lipsync UI flow still dead. (task #28) | IPC inventory | partial (PR #101); 4 handlers pending |
| ~~P0-3~~ | `files.downloadFile` IPC | Renderer calls; no handler exists; **export downloads broken** | IPC inventory | **closed** by loop iter 2 (PR #108) — handler with path-traversal protection + data-dir sandbox + collision-safe naming |
...
| P0-5 | `screenplays.autoFix` + `.autoFixAll` IPC | Renderer calls; no handlers; **screenplay auto-fix dead** | IPC inventory | (pending — needs implementation) |
| ~~P0-6~~ | `sharing.deleteComment` IPC | Renderer calls; only `sharing.resolveComment` exists; **comment delete broken** | IPC inventory | **closed** by loop iter 3 (PR #109) — thin wrapper over `SharingService.delete_comment` |
...
| P0-8 | `analytics.setBudget` + `analytics.getBudget` IPC | Renderer calls both; no handlers exist; **cost-dashboard budget setting dead**. Note: `cost_tracking.set_budget_limit` is in-memory only — fixing correctly needs persistence layer (migration 008 + service refactor; task #34) | IPC inventory + iter 3 investigation | (pending — needs migration 008) |
