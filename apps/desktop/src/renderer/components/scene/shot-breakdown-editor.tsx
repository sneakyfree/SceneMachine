...
export function ShotBreakdownEditor({
  ...
  const updateShot = useCallback(
    (id: string, updates: Partial<Shot>) => {
  ...
  const duplicateShot = useCallback(
    (id: string) => {
      ...
      onShotsChange(newShots.map((s, i) => ({ ...s, shotNumber: i + 1 })));
    },
    [shots, onShotsChange]
  );
  ...
  const generatedCount = useMemo(() => shots.filter((s) => s.generated).length, [shots]);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() =>
              setExpandedIds(
                expandedIds.size === shots.length ? new Set() : new Set(shots.map((s) => s.id))
              )
            }
