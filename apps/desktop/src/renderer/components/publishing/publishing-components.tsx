...
export const UploadProgress: React.FC<{
    ...
    <div className={cn('bg-surface-800 rounded-lg p-4', className)}>
        ...
        <div className="h-2 bg-surface-700 rounded-full overflow-hidden">
            <div
                className={cn(
                    ...
                    state.status === 'complete' ? 'bg-green-500' :
        ...
        <div className="flex justify-between mt-1 text-xs text-surface-400">
            ...
            {state.url && (
                <a href={state.url} target="_blank" rel="noopener noreferrer" className="text-brand-400 hover:underline flex items-center gap-1">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-1 p-2 bg-surface-800 border border-surface-700 rounded-lg min-h-[40px]">
                    {metadata.tags.map((tag) => (
                        <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 bg-brand-500/20 text-brand-400 rounded text-xs">
                            {tag}
...
export function useUpload(onComplete?: (url: string) => void) {
    ...
    const upload = React.useCallback(async (file: File, metadata: VideoMetadata, platforms: Platform[]) => {
