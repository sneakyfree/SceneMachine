...
function computeDelta(from: Snapshot, to: Snapshot): DeltaChange[] {
  ...
  compareObjects(
    from.projectState as Record<string, unknown>,
    to.projectState as Record<string, unknown>
  );
  return changes;
}
              ...
              <button
                ...
                className={cn(
                  ...
                  !isFrom &&
                    !isTo &&
                    !isInRange &&
                    'bg-surface-600 border-surface-500 hover:bg-surface-500'
                )}
  ...
  return (
    <div className="border border-surface-700 rounded-lg overflow-hidden">
      ...
      >
        ...
        <ChevronRight
          className={cn('w-4 h-4 text-surface-400 transition-transform', expanded && 'rotate-90')}
        />
      </button>
...
// Main component
export function DeltaReportsVisualization({ snapshots, onExport, className }: DeltaReportsProps) {
  ...
  if (snapshots.length < 2) {
    return (
      <div className={cn('p-8 text-center', className)}>
        ...
        <p className="text-surface-400">Create at least 2 snapshots to compare changes.</p>
      </div>
    );
  }
