  ...
  return (
    <div className="explainability-dashboard">
      ...
      <div className="view-content">
        {activeView === 'client' && <ClientView pipelineStatus={pipelineStatus} />}
        {activeView === 'operator' && <OperatorView pipelineStatus={pipelineStatus} />}
        {activeView === 'technical' && <TechnicalView logs={actionLogs} onRefresh={fetchLogs} />}
        {activeView === 'audit' && (
          <AuditView
            snapshots={snapshots}
...
const OperatorView: React.FC<{
  pipelineStatus?: ExplainabilityDashboardProps['pipelineStatus'];
}> = ({ pipelineStatus }) => {
      ...
      <div className="view-section">
        ...
        <div className="phase-timeline">
          {phases.map((phase, index) => (
            <div
              key={phase}
              className={`phase-item ${index < currentPhaseIndex ? 'complete' : ''} ${
  ...
  return (
    <div className="audit-view">
      ...
      {deltaReport && (
        <div className="view-section">
          <h3>
            Delta Report: {deltaReport.from_label} → {deltaReport.to_label}
          </h3>
            ...
            <tbody>
              {deltaReport.changes.slice(0, 20).map((change, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <span
                      className={`status-badge ${change.change_type === 'added' ? 'completed' : change.change_type === 'removed' ? 'failed' : 'running'}`}
                    >
                      {change.change_type}
                    </span>
                  </td>
