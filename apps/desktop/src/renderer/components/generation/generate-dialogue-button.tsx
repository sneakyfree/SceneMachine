    ...
    if (compact) {
        return (
            <button
                ...
                className={`p-2 rounded transition-colors flex items-center justify-center ${isProcessing
    ...
    return (
        <div className={`flex flex-col gap-1 ${className}`}>
            <button
                onClick={handleGenerate}
                disabled={isProcessing || !hasDialogue}
                className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${isProcessing
                ...
                aria-label={
                    isProcessing
                        ? `Generating dialogue ${progressText}`
                        : 'Generate dialogue audio'
                }
            >
                {isProcessing ? (
            ...
            {!hasDialogue && !compact && (
                <div className="text-xs text-gray-500 mt-1">
                    No shots have dialogue text to generate.
                </div>
            )}
        </div>
    );
