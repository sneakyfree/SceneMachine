                    ...
                    <div key={stage.id} className="flex items-center">
                        ...
                        >
                            ...
                            <div className="hidden sm:block">
                                <p className={cn(
                                    'text-xs font-medium',
                                    isCompleted ? 'text-green-400' : isCurrent ? 'text-brand-400' : 'text-surface-400'
                                )}>
                                    {stage.label}
                                </p>
                                {isCurrent && (
                                    <p className="text-[10px] text-surface-400">{stage.description}</p>
                                )}
                            </div>
                        </div>
...
export function ProductionDashboard({
    ...
    const { data: status, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const result = await window.electronAPI.backendRequest<PipelineStatus>(
                'pipeline.status',
    ...
    const startMutation = useMutation({
        mutationFn: async () => {
            return await window.electronAPI.backendRequest(
                'pipeline.start',
                { project_id: projectId, mode: 'full_auto' }
            );
        },
        onSuccess: () => {
            setIsRunning(true);
    ...
    useEffect(() => {
        if (status) {
            const activeStages: PipelineStage[] = [
                'parsing', 'shot_breakdown', 'character_prep', 'blocker_check',
                'video_generation', 'quality_review', 'audio_generation', 'lip_sync', 'assembly'
            ];
        ...
        <div className={cn('flex flex-col h-full', className)}>
            ...
            <div className="flex items-center justify-between p-4 border-b border-surface-700">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={() => queryClient.invalidateQueries({ queryKey: ['pipeline-status', projectId] })}
            ...
            <div className="p-4 border-b border-surface-700">
                <StageProgress
                    stages={STAGES}
                    currentStage={status?.stage || 'initialized'}
                />
            </div>
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 border-b border-surface-700">
                    ...
                    <StatCard
                        icon={Clock}
                        label="Time"
                        value={status?.estimatedTimeRemaining ? `${Math.ceil(status.estimatedTimeRemaining / 60)}m` : '--'}
                        subValue="remaining"
                        color="brand"
                    />
                </div>
