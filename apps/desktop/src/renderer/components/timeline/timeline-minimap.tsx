...
export const TimelineMinimap: React.FC<TimelineMinimapProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (clickX >= vpStartPx && clickX <= vpEndPx) {
      ...
      const newStart = Math.max(
        0,
        Math.min(totalDuration - viewportDuration, clickPosition - viewportDuration / 2)
      );
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const newStart = Math.max(
      0,
      Math.min(totalDuration - viewportDuration, dragStart.start + deltaDuration)
    );
...
export function useTimelineViewport(totalDuration: number) {
  ...
  const handleZoomChange = React.useCallback(
    (newZoom: number) => {
      ...
      setViewportStart(
        Math.max(0, Math.min(totalDuration - newDuration, currentCenter - newDuration / 2))
      );
    },
