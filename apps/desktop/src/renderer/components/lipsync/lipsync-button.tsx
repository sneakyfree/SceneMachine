...
export const LipsyncButton: React.FC<LipsyncButtonProps> = ({
    ...
    const { startLipSync, fetchProviders, providers, isLoadingProviders, error } =
        useLipSyncStore();
    ...
    const shotJob = activeJob?.video_id === videoId && activeJob?.audio_id === audioId
        ? activeJob
        : null;
    ...
    return (
        <div className={`flex flex-col gap-1 ${className}`}>
            <button
                ...
                aria-label={
                    shotJob
                        ? `Lip sync in progress: ${shotJob.progress_message}`
                        : buttonState.text
                }
                title={
                    !videoId
