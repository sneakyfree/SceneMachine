...
const PROVIDER_INFO: Record<
  string,
  { description: string; requiresGpu: boolean; icon: React.ReactNode }
> = {
  mock: {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="text-left">
            ...
            {showCapabilities && (
              <div className="text-xs text-surface-500">{selectedMeta.description}</div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-medium flex items-center gap-2">
                          {provider.name}
                          {!provider.available && <AlertCircle className="w-3 h-3 text-red-400" />}
                        </div>
                        <div className="text-xs text-surface-500">{meta.description}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {meta.requiresGpu && (
