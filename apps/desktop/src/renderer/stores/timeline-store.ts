...
interface TimelineActions {
    ...
    createTrack: (projectId: string, data: { name: string; track_type: string; color?: string }) => Promise<Track | null>;
...
export const useTimelineStore = create<TimelineStore>((set, get) => ({
    ...
    createTrack: async (projectId, data) => {
        ...
        try {
            ...
            set({ isSaving: false, error: err instanceof Error ? err.message : 'Failed to create track' });
            return null;
        }
    },
        ...
        try {
            ...
            set((state) => ({
                tracks: state.tracks.map((t) =>
                    t.id === trackId ? { ...updated, clips: t.clips } : t
                ),
                isSaving: false,
            }));
            return updated;
        } catch (err) {
            set({ isSaving: false, error: err instanceof Error ? err.message : 'Failed to update track' });
            return null;
        }
    },
    ...
    reorderTracks: async (projectId, trackIds) => {
        ...
        try {
            ...
            set({ isSaving: false, error: err instanceof Error ? err.message : 'Failed to reorder tracks' });
        }
    },
    ...
    createClip: async (trackId, data) => {
        ...
        try {
            ...
            set((state) => ({
                tracks: state.tracks.map((t) =>
                    t.id === trackId ? { ...t, clips: [...t.clips, clip].sort((a, b) => a.start_time - b.start_time) } : t
                ),
                isSaving: false,
            }));
            return clip;
